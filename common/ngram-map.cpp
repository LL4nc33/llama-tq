...
void common_ngram_map_begin(
    ...
    // When the prompt shrinks below the previous begin index, cleaning up with
    // map.size_last_begin as the cutoff reads/deletes entries out of the valid
    // range. Clamp the cutoff so cleanup stays within [0, size_begin). (#23936)
