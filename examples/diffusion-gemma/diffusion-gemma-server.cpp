...
// Embedded llama-server WebUI assets (generated by xxd.cmake from tools/server/public).
...
#ifdef LLAMA_BUILD_WEBUI
    // Serve the embedded llama-server WebUI on "/". It talks to /props, /v1/models and
    // /v1/chat/completions, all of which this server provides — so the standard chat UI works
    // against the diffusion backend with no client changes.
    http.Get("/", [](const httplib::Request &, httplib::Response & res) {
        res.set_header("Cross-Origin-Embedder-Policy", "require-corp");
        res.set_header("Cross-Origin-Opener-Policy", "same-origin");
        res.set_content(reinterpret_cast<const char *>(index_html), index_html_len, "text/html; charset=utf-8");
    });
    http.Get("/index.html", [](const httplib::Request &, httplib::Response & res) {
        res.set_header("Cross-Origin-Embedder-Policy", "require-corp");
        res.set_header("Cross-Origin-Opener-Policy", "same-origin");
        res.set_content(reinterpret_cast<const char *>(index_html), index_html_len, "text/html; charset=utf-8");
    });
    http.Get("/bundle.js", [](const httplib::Request &, httplib::Response & res) {
        res.set_content(reinterpret_cast<const char *>(bundle_js), bundle_js_len, "application/javascript; charset=utf-8");
    });
    http.Get("/bundle.css", [](const httplib::Request &, httplib::Response & res) {
        res.set_content(reinterpret_cast<const char *>(bundle_css), bundle_css_len, "text/css; charset=utf-8");
    });
    http.Get("/loading.html", [](const httplib::Request &, httplib::Response & res) {
