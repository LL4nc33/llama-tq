...
3. **🟠 Multi-GPU split disagreement** (Section 1.5). Probability MEDIUM (test-rig is the deploy target). Impact HIGH (silent quality regression hard to detect). Mitigation: design the dynamic-mask op to be per-token-deterministic regardless of which GPU computes the router (router runs on GPU0, mask broadcast via existing tensor split).
