...
Stubbed in `fattn-vec.cuh:353-364` and `fattn-vec.cuh:380-394` — replaced both `dequantize_V` calls with `tmp[i] = make_half2(0,0)` / `make_float2(0,0)`. File reverted locally + scp'd back to test-rig. Not committed.
