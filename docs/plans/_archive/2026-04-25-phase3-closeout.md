...
Production deploy on internal-host:8791 (80B) and 8794 (122B) was running `vtq2_1` since
...
- **Concept is portable**: same goal achievable on NVIDIA via different tools:
  ...
  5. Headless test-rig (Xorg off, no compositor → no eviction races)
...
**Discovery:** test-rig is a KVM guest VM (12 vCPUs from a Ryzen 7 3700X host),
...
4. NVIDIA exclusive-process mode + headless test-rig (Valve-fix equivalent).
...
```bash
ssh <user>@example.local "uname -r; cat /etc/os-release | head -3; \
