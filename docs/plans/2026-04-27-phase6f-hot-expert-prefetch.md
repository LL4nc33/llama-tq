...
6. **CCX pinning required:** Without `taskset`/cpuset to one CCX, threads bounce → L3 effect halves. Deploy script must pin CPU threads (`--threads 8 --cpu-mask 0xFF`) to a single CCX. test-rig is a KVM guest; host-side cpuset may also be needed for full effect.
