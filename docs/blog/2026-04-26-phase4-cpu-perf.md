...
**Date:** 2026-04-26
**Branch:** turboquant
**Hardware:** example.local — KVM guest (12 vCPUs from Ryzen 7 3700X), 40 GB DDR4-3200, 2× RTX 2060 12 GB on asymmetric PCIe x16/x4
...
We started by checking `/proc/cpuinfo` and discovered test-rig is a KVM guest VM (`systemd-detect-virt = kvm`). The Linux kernel inside the guest sees one virtual L3 cache shared by all 12 vCPUs — real CCX topology of the underlying Ryzen 7 3700X is hidden by the hypervisor.
