...
Both production deployments (`localhost:8791` for 80B, port 8794 for 122B) currently run with `--cache-type-v vtq2_1` — the v1 PolarQuant V-cache. We have measured today that switching to `vtq2_2` (the v2 Trellis-coded V-cache) **gives a free quality upgrade** on both models, while saving 0.22 bpw.
