...
- Researcher / sub-agent line-number reports may come from a stale clone.
  Always verify with `ssh <user>@example.local "sed -n 'L,Lp' README.md"` before
  applying edits.
...
- `example.local:8791` — `OidaNice-GPT-34B` (Qwen3.6-35B-A3B-IQ2_XXS hardlink),
...
- `example.local:8793` — `nomic-embed-text-v1.5` F16 embeddings server, GPU1, 380 MB,
  systemd-user service with autostart.
- `example.local:8792` — `functiongemma-270m`, CPU.
- VM `on-agents` (<vm-host>, user `claude`) hosts opencode + Hermes Agent +
...
## 11. Build & Revert Workflow (test-rig) — ALWAYS APPLIES
