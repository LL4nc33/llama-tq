...
PORT=8791
MODEL=/models/models/gpt-oss-20b-F16.gguf
LLAMA_BIN=/workspace/llama-tq/build/bin/llama-server
SLOTS=/workspace/llama-slots-20b/
...
CUDA_VISIBLE_DEVICES=0,1 OMP_WAIT_POLICY=active OMP_PROC_BIND=close OMP_PLACES=cores \
  ...
  --jinja --flash-attn on \
  -c 131072 -ngl 99 -ts 16,8 --no-mmap --parallel 1 \
  ...
  --override-kv general.name=str:OidaNice-GPT-20B \
