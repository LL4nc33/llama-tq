...
// NOTE: TurboQuant helpers (PolarQuant codebooks, FWHT decoders, KTQ/VTQ
// vec-dot + V-dequant, type dispatchers) live in fattn-tq.cuh. Including
// turboquant.cuh + trellis.cuh from this header was found to leak ~1700
...
// ============================================================
// TurboQuant FA helpers (KTQ/VTQ vec-dot, V-dequant, dispatchers)
// moved to fattn-tq.cuh — see TU-bloat note at top of this file.
// ============================================================
...
// get_vec_dot_KQ<type_K, ...> and get_dequantize_V<type_V, ...> live in
// fattn-tq.cuh (they reference TQ helper templates). TUs that need to
// pick a dispatch function pointer for a TQ ggml_type must include
// fattn-tq.cuh; non-TQ FA paths never reach the constexpr dispatcher.
...
// XQuant Phase 3d — paired flash-attention launcher.
//
// IRON-LAW: This is a SEPARATE function from launch_fattn(). It is only called
// from the paired-vec wrapper (ggml_cuda_flash_attn_ext_vec_case_paired_impl)
// for XKTQ subordinate K types. All non-paired backends (mma-f16, mma-ktq,
// tile, wmma-f16, vec, vec-vtq2) continue to call the original launch_fattn
// — its body is byte-identical to before this commit.
//
// Body is duplicated from launch_fattn() above, with two differences:
//   1. Kernel pointer type is fattn_kernel_paired_t (forward-declared below
//      to avoid pulling fattn-vec-dispatch.cuh into fattn-common.cuh).
//   2. Kernel launch passes an extra K_dom pointer as the final argument.
//
// The duplication is intentional: extracting a shared helper would require
// either (a) changing launch_fattn's signature (touches every existing
// backend, violating Iron-Law) or (b) a templated dispatch that adds compile
// time to existing TUs. Both unacceptable. ~150 LOC of duplication is the
// price of zero-impact on stable code paths.
typedef void (* fattn_kernel_paired_fwd_t)(
        const char * __restrict__ Q,
        const char * __restrict__ K,
        const char * __restrict__ V,
        const char * __restrict__ mask,
...
template <int DV, int ncols1, int ncols2>
void launch_fattn_paired(
    ggml_backend_cuda_context & ctx, ggml_tensor * dst, fattn_kernel_paired_fwd_t fattn_kernel,
    ...
    // Sibling dominant-K tensor (Phase 3b). Caller must ensure non-null.
    const ggml_tensor * K_dom = dst->src[5];
    GGML_ASSERT(K_dom != nullptr && "launch_fattn_paired requires dst->src[5]");
    ...
    // Paired path is only used for XKTQ subordinate K → never f16-K conversion.
    // Mirror the f16-K/V conversion logic for safety/symmetry.
    if (need_f16_K && K->type != GGML_TYPE_F16) {
        const size_t bs = ggml_blck_size(K->type);
        const size_t ts = ggml_type_size(K->type);
