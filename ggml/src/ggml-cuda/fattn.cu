...
// Split-dequant fallback for MMA-KTQ. Bulk-dequantizes the KTQ K-cache into
// fp16 scratch, swaps K tensor metadata, dispatches to mma_f16, and restores
// metadata on exit. Lives here (not in fattn-mma-ktq.cu) so the call to
// ggml_cuda_flash_attn_ext_mma_f16 is intra-TU; with CUDA_SEPARABLE_COMPILATION
// nvcc otherwise demotes that symbol to local linkage when it is also
// referenced inside fattn.cu, which breaks the cross-TU call from
// fattn-mma-ktq.cu's own ktq_split.
void ggml_cuda_flash_attn_ext_mma_ktq_split(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    // vx, which is then offset by sizeof(block_ktq*_1) via `x[ib]`).
