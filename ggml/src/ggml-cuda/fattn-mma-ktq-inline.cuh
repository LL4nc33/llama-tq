...
static __device__ __forceinline__ void flash_attn_ext_f16_ktq_load_tile(
...
// the downstream MMA path expects from half2 tile fills).
template<int stride_tile, int nwarps, int nbatch_fa, bool oob_check>
static __device__ __forceinline__ void flash_attn_ext_f16_ktq_load_tile_K_ktq2_1(
        const block_ktq2_1 * const __restrict__ K_blocks,
        half2 * const __restrict__ tile_K,
        const int D2,                 // head_dim in half2 (= head_dim/2)
        const int stride_K_blocks,    // row stride in block_ktq2_1 units
        const int i_sup) {
    const int tid  = threadIdx.x;
    const int warp = threadIdx.y;
    const int D    = 2 * D2;
    const int nblk_per_row = D / QK_KTQ;
    ...
    for (int i0 = 0; i0 < nbatch_fa; i0 += nwarps) {
        ...
        const bool oob = oob_check && (i >= i_sup);
        const block_ktq2_1 * row = K_blocks + int64_t(i) * stride_K_blocks;
        half * tile_row_h = reinterpret_cast<half *>(tile_K + i * stride_tile);
...
#pragma unroll
    for (int k0_start = (DKQ/2-1) - (DKQ/2-1) % nbatch_K2; k0_start >= 0; k0_start -= nbatch_K2) {
        ...
        if constexpr (nstages <= 1) {
            constexpr bool use_cp_async = nstages == 1;
            // KTQ inline: ignore k0_start/k0_diff slicing — KTQ load dequants the
            // whole row in one warp sweep. The downstream MMA reads from shmem
            // the same half2 layout the f16 path produces.
            flash_attn_ext_f16_ktq_load_tile_K_ktq2_1<stride_tile_K, nwarps, nbatch_fa, oob_check>
                (K_ktq + int64_t(k_VKQ_0)*stride_K, tile_K, DKQ/2, stride_K, k_VKQ_sup);
            if (use_cp_async) {
                cp_async_wait_all();
            }
            __syncthreads();
...
#pragma unroll
    ...
    if constexpr (nstages > 1) {
        ...
        if (!last_iter) {
            ...
            flash_attn_ext_f16_ktq_load_tile_K_ktq2_1<stride_tile_K, nwarps, nbatch_fa, oob_check>
                (K_ktq + int64_t(k_VKQ_0 + nbatch_fa)*stride_K, tile_K, DKQ/2, stride_K, k_VKQ_sup);
        }
    }
...
#else
    GGML_UNUSED_VARS(Q_f2, K_ktq, V_h2, mask_h, dstk, dstk_fixup,
...
#pragma unroll
    ...
    if constexpr (nstages > 1) {
        ...
        flash_attn_ext_f16_ktq_load_tile_K_ktq2_1<stride_tile_K, nwarps, nbatch_fa, oob_check>
            (K_ktq + int64_t(kb0)*nbatch_fa*stride_K, tile_K, DKQ/2, stride_K, k_VKQ_sup);
    }
    ...
    if constexpr (ncols2 == 1) {
        ...
        for (; kb0 < kb0_stop-1; ++kb0) {
            ...
            constexpr int  k_VKQ_sup = nbatch_fa;
            flash_attn_ext_f16_ktq_iter
                <DKQ, DV, ncols1, ncols2, nwarps, use_logit_softcap, V_is_K_view, needs_fixup, is_fixup, last_iter, oob_check,
                 T_A_KQ, T_B_KQ, T_C_KQ, T_A_VKQ, T_B_VKQ, T_C_VKQ, V_is_vtq2_1>
                (Q_f2, K_ktq, V_h2, mask_h, dstk, dstk_fixup, scale, slope, logit_softcap,
...
#else
    GGML_UNUSED_VARS(Q_f2, K_ktq, V_h2, mask_h, sinks_f, dstk, dstk_fixup,
...
static __global__ void flash_attn_ext_f16_ktq_kernel(
    ...
    while (kbc < kbc_stop && kb0_stop == iter_k) {
        ...
        const block_ktq2_1 * K_ktq = (const block_ktq2_1 *) (K + nb13*sequence + nb12*z_KV);
        const half   * mask_h = ncols2 == 1 && !mask ? nullptr :
            (const half *) (mask + nb33*(sequence % ne33));
        float2       * dstk   = ((float2 *) dst) + (sequence*ne01.z*ne02 + zt_Q) * (DV/2);

        const half2 * V_h2 = (const half2 *) (V + nb23*sequence + nb22*z_KV); static_assert(!V_is_K_view, "V_is_K_view not supported for KTQ inline");
        ...
        if (kb0_start == 0) {
            ...
            flash_attn_ext_f16_ktq_process_tile<DKQ, DV, ncols1, ncols2, nwarps, use_logit_softcap, V_is_K_view, needs_fixup, is_fixup, V_is_vtq2_1>
                (Q_f2, K_ktq, V_h2, mask_h, sinks_f, dstk, dst_meta, scale, slope, logit_softcap,
...
void ggml_cuda_flash_attn_ext_mma_ktq_inline_case(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    if (logit_softcap == 0.0f) {
        ...
        fattn_kernel = flash_attn_ext_f16_ktq_kernel<DKQ, DV, ncols1, ncols2, use_logit_softcap, V_is_K_view, V_is_vtq2_1>;
...
#define DECL_FATTN_MMA_KTQ_INLINE_CASE(DKQ, DV, ncols1, ncols2)                          \
    template void ggml_cuda_flash_attn_ext_mma_ktq_inline_case                           \
    <DKQ, DV, ncols1, ncols2>(ggml_backend_cuda_context & ctx, ggml_tensor * dst) \

// VTQ variant: explicit V_is_vtq2_1=true. Used when V cache is vtq2_1.
#define DECL_FATTN_MMA_KTQ_VTQ_INLINE_CASE(DKQ, DV, ncols1, ncols2)                       \
    template void ggml_cuda_flash_attn_ext_mma_ktq_inline_case                            \
    <DKQ, DV, ncols1, ncols2, /*V_is_vtq2_1=*/true>(ggml_backend_cuda_context & ctx, ggml_tensor * dst) \

#define DECL_FATTN_MMA_KTQ_INLINE_CASE_ALL_NCOLS2(DKQ, DV, ncols)   \
    extern DECL_FATTN_MMA_KTQ_INLINE_CASE(DKQ, DV, (ncols)/ 1,  1); \
    extern DECL_FATTN_MMA_KTQ_INLINE_CASE(DKQ, DV, (ncols)/ 2,  2); \
    extern DECL_FATTN_MMA_KTQ_INLINE_CASE(DKQ, DV, (ncols)/ 4,  4); \
    extern DECL_FATTN_MMA_KTQ_INLINE_CASE(DKQ, DV, (ncols)/ 8,  8); \
    extern DECL_FATTN_MMA_KTQ_INLINE_CASE(DKQ, DV, (ncols)/16, 16); \
...
extern DECL_FATTN_MMA_KTQ_INLINE_CASE(128, 128, 4, 4);
extern DECL_FATTN_MMA_KTQ_INLINE_CASE(128, 128, 8, 4);
