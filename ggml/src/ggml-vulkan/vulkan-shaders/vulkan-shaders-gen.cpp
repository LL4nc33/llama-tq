...
const std::vector<std::string> type_names = {
    ...
    "nvfp4",
    "bf16",
    "ktq2_1", // TurboQuant K-cache 2-bit (3.5 bpw, QK=32, RHT+sign+FWHT). V1: KTQ2_1-only.
};
...
void matmul_shaders(bool fp16, MatMulIdType matmul_id_type, bool coopmat, bool coopmat2, bool f16acc) {
    ...
    for (const auto& tname : type_names) {
        ...
        if ((tname == "q1_0") || (tname == "q4_0") || (tname == "q4_1") || (tname == "q5_1") || (tname == "iq1_s") || (tname == "iq1_m") || (tname == "iq2_xxs") || (tname == "iq2_xs") || (tname == "iq2_s"))
            load_vec_quant = "8";
        // ktq2_1: KV-only, no MMV path — leave default load_vec_quant=2.
    ...
    for (const bool& fp16 : {false, true}) {
        ...
        for (const bool& f16acc : {false, true}) {
            ...
            for (const auto& tname : type_names) {
                ...
                // KTQ2_1: cm1/cm2 not yet wired (no dequantFuncKTQ2_1 in dequant_funcs_cm2.glsl
                // and no cm1 entry in dequant_funcs.glsl). A-cm-paths agent owns S4/S5.
                // Scalar FA pipeline is still generated by the unconditional
                // flash_attn.comp branch further down.
                const bool is_ktq2_1 = (tname == "ktq2_1");
                ...
                } else if (tname == "q4_0" || tname == "q4_1" || tname == "q5_0" || tname == "q5_1" || tname == "iq4_nl" || tname == "q8_0" || tname == "f32" || tname == "ktq2_1") {
                    std::string data_a_key = "DATA_A_" + to_uppercase(tname);
                    string_to_spv("flash_attn_f32_f16_" + tname, "flash_attn.comp",
...
#if defined(GGML_VULKAN_INTEGER_DOT_GLSLC_SUPPORT)
                    // KTQ2_1: skip the int8/MMQ path — its qs[] holds 2-bit codebook indices, not signed q8 values.
                    if (tname != "f32" && tname != "ktq2_1") {
                        string_to_spv("flash_attn_f32_f16_" + tname, "flash_attn.comp",
